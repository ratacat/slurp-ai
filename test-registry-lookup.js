...
// Create test examples for the LocalRegistryLookup
async function runTests() {
  const registryLookup = new LocalRegistryLookup();
  
  console.log('\n--------- LocalRegistryLookup Test ---------\n');
  ...
  try {
    ...
    for (const pkg of packages) {
      ...
      const result = await registryLookup.findPackageDoc(pkg);
    ...
    for (const test of versionTests) {
      ...
      const result = await registryLookup.findVersionDocUrl(test.package, test.version);
    ...
    console.log('\n\nTest 3: Analyze package.json dependencies');
    const dependencies = await registryLookup.analyzeDependencies(path.join(__dirname, 'package.json'));
